...
class Benchmark:
    def __init__(self, savefile='benchmark.csv',pid=0):
    ...
    def header(self):
        ...
        f.write("pid;frame;cpu;memory;task\n")
        f.close()

    def saveBenchmark(self,frame,task):
        mem, cpu = self.collect()
        f = open(self.savefile, "a")
        f.write(str(self.pid)+";"+str(frame)+";"+str(cpu)+";"+str(mem)+";"+task+"\n")
        f.close()
...
while(True):
    ...
    ret, frame = cap.read()
    benchmark.saveBenchmark(count,'begin')
    if sampling == frameSampling:
        ...
        benchmark.saveBenchmark(count,'cnn')
	...
	if prob > confidenceVerifier:
		...
		scikit = ScikitLearn()
                ...
                benchmark.saveBenchmark(count,'scikit')
    ...
    count += 1
    sampling += 1
    benchmark.saveBenchmark(count,'end')
    if count == 100:
        break
